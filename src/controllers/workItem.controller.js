...
async function startWorkItem(request, reply) {
  ...
  let workflowId = request.params.id || (request.body && request.body.id);
  console.log(`Received workflowId: ${workflowId} (Type: ${typeof workflowId})`);

  console.log(`Starting work item. Received workflowId: ${workflowId}`);
...
async function completeWorkItem(request, reply) {
  const workItemId = request.params.id || request.body.id;
